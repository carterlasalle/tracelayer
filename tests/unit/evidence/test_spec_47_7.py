...
def _exec_edge(run_id, test_uid, impl_uid, kind, hit_count):
    ...
    return ExecutionRecord(
        run_id=run_id, test_uid=test_uid, implementation_uid=impl_uid,
        coverage_kind=kind, hit_count=hit_count,
    )
...
def req_impl_test_graph(store, *, test_trace_id="TEST:1", test_active=True):
    ...
    test = make_node(
        test_trace_id, "test", path="tests/test_app.py", active=test_active
    )
    store.replace_all(
        [req, impl, test],
        [
...
def test_deleted_test_trace_clean_after_edges_retired(project, store):
    ...
    result = evaluate(
        project, store, lifecycle="merge",
        changed_ids={"TEST:1", "IMPL:1", "REQ:1"}, revision="abc123",
    )
